# part 0. (uncomment/comment lines at the top depending on whether running at
# home or at work!)
...
# part 3. concatenate two datasets
# table(test$subject)
# table(train$subject)
...
# part 4. keep only mean and std measurements
...
# Part 5. merge in activity from activity labels
...
# part 6. Use aggregate to make finalSet. 

summaryTidy <- aggregate(tidySet,by=list(subject = tidySet$subject,activity = tidySet$activity),mean)
head(summaryTidy)
finalSet <- summaryTidy[,2:82] # drop extra vars
head(finalSet)

# part 7. export it (by uncommenting)
